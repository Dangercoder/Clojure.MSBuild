...
(defactor ant
  ...
  (init [self id]                                       ; state on activation
    (let [{:keys [x y dir]} (t/await (actors/call (world "main") :spawn id))]
      {:id id :x x :y y :dir dir :food? false}))

  (call :state [self state]                             ; request/response
    (reply state state))

  (call :step [self {:keys [x y dir] :as state}]
    (let [look (t/await (actors/call (world "main") :look {:x x :y y :dir dir}))]
      ...
      (reply :moved (assoc state :x nx :y ny)))))
```
...
- `test/ants/logic_test.cljr` the rules; `test/ants/colony_test.cljr` runs the
  actors on a real silo (own ports, so it can run next to `dotnet run`).
